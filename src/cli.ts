...
// When --publish is given, generates a self-contained dark-styled changelog.html instead of Markdown.
...
async function main() {
  ...
  if (publish) {
    // HTML publish mode — generate dark-styled self-contained changelog.html
    const html = generateHtml(filtered);
    const htmlFile = outFile
      ? outFile.replace(/(\.[^.]+)?$/, ".html")
      : "changelog.html";
    await Bun.write(htmlFile, html + "\n");
    console.error(`changelog published to ${htmlFile}`);
    return;
  }
