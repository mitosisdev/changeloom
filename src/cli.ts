...
//
// Usage:
//   bun src/cli.ts [repo-path] [--version v1.2.3] [--out <file>] [--since <ref>] [--types <type1,type2>]
//
...
export function parseArgs(
  argv: string[],
): { repoPath: string; version?: string; outFile?: string; since?: string; types?: string[] } {
...
async function main() {
  ...
  const filtered = types && types.length > 0 ? filterByTypes(commits, types) : commits;
